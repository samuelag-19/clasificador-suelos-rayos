...
# ────────────────────────────────────────────────────────
# CARGAR DATOS (con cache para que no se recargue cada vez)
# ────────────────────────────────────────────────────────

@st.cache_resource
def cargar_datos():
    """Carga GeoPackage de suelos e HDF de rayos"""
    try:
        gdf = gpd.read_file("datos/regionesunidas.gpkg")
        hdf = SD("datos/ISSAnnualMean.hdf", SDC.READ)
